...
pub const DashboardView = struct {
    ...
    fn draw(self: *DashboardView, ctx: vxfw.DrawContext) !vxfw.Surface {
        ...
        try writeString(&surface, ctx, 2, row, subtitle, styles.styles.muted);
        row += 2;
        ...
        try writeString(&surface, ctx, 2, row, refresh_label, refresh_style);
        row += 2;

        // Statistics section
        try writeString(&surface, ctx, 2, row, "BLOCKCHAIN STATS", styles.styles.title);
        row += 1;
        try drawLine(&surface, row, max_size.width, styles.styles.muted);
        row += 1;

        // Stats grid
        try drawStatRow(&surface, ctx, row, 2, "Block Height", self.stats.block_height);
        try drawStatRow(&surface, ctx, row, 30, "Total Blocks", self.stats.total_blocks);
        row += 1;

        try drawStatRow(&surface, ctx, row, 2, "Transactions", self.stats.total_transactions);
        try drawStatRow(&surface, ctx, row, 30, "Successful", self.stats.successful_txs);
        row += 1;

        try drawStatRow(&surface, ctx, row, 2, "Failed Txs", self.stats.failed_txs);
        try drawStatRow(&surface, ctx, row, 30, "Gas Used", self.stats.total_gas_used);
        row += 1;

        try drawStatRow(&surface, ctx, row, 2, "Accounts", @as(u64, self.stats.total_accounts));
        try drawStatRow(&surface, ctx, row, 30, "Contracts", @as(u64, self.stats.total_contracts));
        row += 2;

        // Recent blocks section
        try writeString(&surface, ctx, 2, row, "RECENT BLOCKS", styles.styles.title);
        row += 1;
        try drawLine(&surface, row, max_size.width, styles.styles.muted);
        row += 1;
        ...
        try writeString(&surface, ctx, 2, row, "RECENT TRANSACTIONS", styles.styles.title);
        row += 1;
        try drawLine(&surface, row, max_size.width, styles.styles.muted);
        row += 1;
        ...
        if (self.recent_txs.len == 0) {
            ...
            for (self.recent_txs) |tx| {
                ...
                try writeString(&surface, ctx, 2, row, tx_str, tx_style);
                row += 1;
...
fn drawStatRow(surface: *vxfw.Surface, ctx: vxfw.DrawContext, row: u16, col: u16, label: []const u8, value: u64) !void {
