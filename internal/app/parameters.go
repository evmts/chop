...
func GetCallParams(cp types.CallParametersStrings) []types.CallParameter {
	...
	paramConfig := []struct {
		...
		},
		{
			...
			value:    cp.Target,
			showWhen: func(t string) bool { return t != "CREATE" && t != "CREATE2" },
		},
		{
			name:     config.CallParamValue,
			value:    cp.Value,
			showWhen: func(t string) bool { return t != "STATICCALL" },
		},
		{
		...
		},
		{
			...
			value:    cp.Salt,
			showWhen: func(t string) bool { return t == "CREATE2" },
		},
	}
